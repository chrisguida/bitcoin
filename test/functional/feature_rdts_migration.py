...
(-powchangetime) must signal; the fork itself ends the requirement.
...
RDTS_ENFORCE = [f'-powchangetime={FAR_FORK}:1', '-rdtssignalwindow=288:432']
WINDOW_START = 288
...
RDTS_POSTFORK = [f'-powchangetime={TIME_GENESIS_BLOCK + POSTFORK_CROSS_HEIGHT}:1',
...
RDTS_PRUNE = [f'-powchangetime={FAR_FORK}:1', '-rdtssignalwindow=144:288']
PRUNE_VIOLATOR = 144
...
class RdtsMigrationTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # genesis + height, and node4's -powchangetime crosses at height 300).
