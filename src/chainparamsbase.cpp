...
void SetupChainParamsBaseOptions(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-hardforktime=<time>", "Schedule the hardfork at the given block time. (regtest-only)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    argsman.AddArg("-rdtsexpiry=<time>", "Schedule the end of the RDTS flag day: RDTS rules apply to blocks with time in [hardforktime, expiry). The activation time is -hardforktime, which is required. (regtest-only)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    argsman.AddArg("-rdtssignalwindow=<begin>:<end>", "Require blocks in the [begin, end) height window whose time precedes -hardforktime to signal the RDTS versionbit. Requires -hardforktime. (regtest-only)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
