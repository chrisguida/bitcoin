...
void SetupChainParamsBaseOptions(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-rdtsexpiry=<time>", "Schedule the end of the RDTS flag day: RDTS rules apply to blocks with time in [powchangetime, expiry). The activation time is -powchangetime, which is required. (regtest-only)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    argsman.AddArg("-rdtssignalwindow=<begin>:<end>", "Require blocks in the [begin, end) height window whose time precedes -powchangetime to signal the RDTS versionbit. Requires -powchangetime. (regtest-only)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
