...
once the spending block's parent has a median-time-past at least 45 days
(3,888,000 seconds) past the coinbase block's median-time-past,
on top of the usual 100 confirmations. Measuring the delay in median-time-past
rather than blocks keeps it 45 days of real time however fast or slow
blocks come, and a miner cannot shorten it: block times cannot run more than
about two hours ahead of real time, so neither can the median.
...
RDTS expiry (2027-09-01 00:00 UTC on mainnet) every coinbase output is again
spendable after 100 confirmations, including those created during the window
that had not yet served their 45 days.
...
Miners: block rewards mined inside the window cannot be spent for 45 days
...
so the wallet never creates a transaction the network would reject. The
"blocks to maturity" the wallet reports for such a reward is an estimate,
the remaining time at the target block spacing.
